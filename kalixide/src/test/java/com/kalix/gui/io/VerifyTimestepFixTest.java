...
class VerifyTimestepFixTest {
    ...
    void testLoadActualData() throws IOException {
        ...
        // Load all series
        List<NamedSeries> loadedSeries = reader.readAllSeries(resourcePath);
        ...
        for (int i = 0; i < loadedSeries.size(); i++) {
            NamedSeries series = loadedSeries.get(i);
            assertTrue(series.name().contains("rex_rain"), "Series " + (i+1) + " name should contain 'rex_rain'");
