...
public class ParametersConfigPanel extends JPanel {
    ...
    /**
     * Builds parameter entries for a fresh list of parameter names, auto-generating
     * a calibration expression for each recognised parameter type. Unrecognised
     ...
     */
    public static java.util.List<OptimisationConfigModel.ParamEntry> buildAutoGeneratedEntries(
            java.util.List<String> parameterNames) {
        java.util.List<OptimisationConfigModel.ParamEntry> entries = new ArrayList<>();
        ...
        for (String name : parameterNames) {
            String expression = "";
            try {
                expression = ParameterExpressionLibrary.generateExpression(name, geneIndex);
                geneIndex++;
            } catch (ParameterExpressionLibrary.UnrecognizedParameterTypeException e) {
                // Leave blank — the user can add an expression manually.
            }
            entries.add(new OptimisationConfigModel.ParamEntry(name, expression));
        }
        return entries;
    }

    /**
     * Returns every row of the parameters table, in order, including parameters
     * with a blank expression. Unlike {@link #getOptimizationParameters()} this
     * captures the full table state so the GUI form can be restored exactly.
     *
     * @return ordered list of (parameter name, expression) entries
     */
    public java.util.List<OptimisationConfigModel.ParamEntry> getAllParameters() {
        if (paramsTable.isEditing()) {
            paramsTable.getCellEditor().stopCellEditing();
        }
        java.util.List<OptimisationConfigModel.ParamEntry> out = new ArrayList<>();
        for (int i = 0; i < paramsTableModel.getRowCount(); i++) {
            out.add(new OptimisationConfigModel.ParamEntry(
                paramsTableModel.getParameterNameAt(i),
                paramsTableModel.getExpressionAt(i)));
        }
        return out;
    }

    /**
     * Replaces the entire parameters table with the given entries, restoring both
     * the parameter names and their expressions.
     *
     * @param params ordered list of (parameter name, expression) entries
     */
    public void setParameters(java.util.List<OptimisationConfigModel.ParamEntry> params) {
        if (params == null) {
            return;
        }
        ...
        paramsTableModel = new OptimizationParamsTableModel(names);
        paramsTable.setModel(paramsTableModel);
        for (int i = 0; i < params.size(); i++) {
            paramsTableModel.setExpressionAt(i, params.get(i).expression);
        }
