...
use crate::hydrology::rainfall_runoff::surm::Surm;
use crate::misc::location::Location;
use crate::model_inputs::DynamicInput;
use crate::numerical::opt::optimisable_component::OptimisableComponent;

const MAX_DS_LINKS: usize = 1;
const PARAM_NAMES: [&str; 9] = ["imp_fraction", "impsc", "smsc", "coeff", "sq", "fc", "rfac", "bfac", "sfac"];

#[derive(Clone)]
pub struct SurmNode {
    pub name: String, pub location: Location, pub mbal: f64,
    pub rain_mm_input: DynamicInput, pub evap_mm_input: DynamicInput,
    pub area_km2: f64, pub surm_model: Surm, usflow: f64, dsflow_primary: f64,
    pub dsorders: [f64; MAX_DS_LINKS],
    recorder_idx_usflow: Option<usize>, recorder_idx_runoff_volume: Option<usize>,
    recorder_idx_runoff_depth: Option<usize>, recorder_idx_dsflow: Option<usize>,
    recorder_idx_ds_1: Option<usize>, recorder_idx_ds_1_order: Option<usize>,
    recorder_idx_rain: Option<usize>, recorder_idx_evap: Option<usize>,
}

impl Default for SurmNode {
    fn default() -> Self {
        Self {
            name: String::new(), location: Location::default(), mbal: 0.0,
            rain_mm_input: DynamicInput::default(), evap_mm_input: DynamicInput::default(),
            area_km2: 1.0, surm_model: Surm::new(), usflow: 0.0, dsflow_primary: 0.0,
...
impl SurmNode { pub fn new() -> Self { Self::default() } }

impl Node for SurmNode {
    single_outlet_node_impls!();
    fn initialise(&mut self, data_cache: &mut DataCache, _accounts: &mut AccountManager) -> Result<(), String> {
        self.mbal = 0.0; self.usflow = 0.0; self.dsflow_primary = 0.0; self.surm_model.reset();
        if self.area_km2 < 0.0 { return Err(format!("Error in node '{}'. Catchment area cannot be negative, but was {}.", self.name, self.area_km2)); }
        ...
        self.recorder_idx_evap = recorder(data_cache, &self.name, "evap"); Ok(())
    }
    ...
    fn run_flow_phase(&mut self, data_cache: &mut DataCache, _accounts: &mut AccountManager) {
        ...
        let rain = self.rain_mm_input.get_value(data_cache); let pet = self.evap_mm_input.get_value(data_cache);
        let runoff_depth = self.surm_model.run_step(rain, pet); let runoff_volume = runoff_depth * self.area_km2;
        self.dsflow_primary = self.usflow + runoff_volume; self.mbal += runoff_volume;
...
impl OptimisableComponent for SurmNode {
    fn set_param(&mut self, name: &str, value: f64) -> Result<(), String> {
        ...
        if let Some(i) = PARAM_NAMES.iter().position(|&n| n == name) {
            let mut params = self.surm_model.get_params_as_vec(); params[i] = value;
            self.surm_model.set_params_by_vec(&params); Ok(())
        } else { Err(format!("Unknown SURM parameter: {}", name)) }
    }
    fn get_param(&self, name: &str) -> Result<f64, String> {
        ...
        PARAM_NAMES.iter().position(|&n| n == name).map(|i| self.surm_model.get_params_as_vec()[i])
            .ok_or_else(|| format!("Unknown SURM parameter: {}", name))
    }
