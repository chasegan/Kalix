...
2. kalixcli generates a unique kalixcli_uid and sends ready signal with available commands
3. Frontend receives kalixcli_uid and uses it for all subsequent messages
...
```json
{
  ...
  "kalixcli_uid": "unique_kalixcli_uidentifier",
  "data": {}
}
```

**Note**: The `kalixcli_uid` is generated by kalixcli when it starts and included in all kalixcli → frontend messages for logging/debugging purposes. Frontend messages do not need to include kalixcli_uid as each kalixcli process handles exactly one session.
...
Sent when kalixcli starts (with newly generated kalixcli_uid) and after each command completion/interruption.

```json
{
  "type": "ready",
  "timestamp": "2025-09-08T10:30:00Z",
  "kalixcli_uid": "sess_20250908_103000_a7b9",
  "data": {
    "status": "ready",
    "available_commands": [
      {
...
Frontend messages do not need to include kalixcli_uid (each kalixcli process handles one session).
...
- Frontend does not need to include kalixcli_uid in messages (each process = one session)
...
- Frontend does not need to track or send kalixcli_uid
