...
/// Optimisable parameter names, in the order used by `Surm::set_params_by_vec`.
...
impl Node for SurmNode {
    ...
    fn initialise(&mut self, data_cache: &mut DataCache, _account_manager: &mut AccountManager) -> Result<(), String> {
        ...
        // Initialize the SURM model
        self.surm_model.reset();
        ...
        if let Err(message) = self.surm_model.validate_params() {
    ...
    fn run_flow_phase(&mut self, data_cache: &mut DataCache, _account_manager: &mut AccountManager) {
        ...
        // Run SURM model to get runoff
        let runoff_depth_mm = self.surm_model.run_step(rain, pet);
...
impl OptimisableComponent for SurmNode {
    fn set_param(&mut self, name: &str, value: f64) -> Result<(), String> {
        ...
        // Standard SURM parameters
        match PARAM_NAMES.iter().position(|&n| n == name) {
            Some(i) => {
                let mut params = self.surm_model.get_params_as_vec();
                params[i] = value;
                self.surm_model.set_params_by_vec(&params);
                Ok(())
            },
            None => Err(format!("Unknown SURM parameter: {}", name)),
        }
    }
    ...
    fn get_param(&self, name: &str) -> Result<f64, String> {
        ...
        // Standard SURM parameters
        match PARAM_NAMES.iter().position(|&n| n == name) {
            Some(i) => Ok(self.surm_model.get_params_as_vec()[i]),
            None => Err(format!("Unknown SURM parameter: {}", name)),
        }
    }
