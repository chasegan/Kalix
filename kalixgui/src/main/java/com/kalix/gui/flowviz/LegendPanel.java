...
public class LegendPanel extends JPanel {
    ...
    private List<SeriesLegendItem> legendItems;
    ...
    private SeriesLegendItem selectedItem = null;
    ...
    public void addSeries(String seriesName, Color seriesColor, int pointCount) {
        ...
        SeriesLegendItem item = new SeriesLegendItem(seriesName, seriesColor, pointCount, 
            () -> {
    ...
    public void removeSeries(String seriesName) {
        legendItems.removeIf(item -> item.getSeriesName().equals(seriesName));
        updateLegendDisplay();
    }
    ...
    private void updateLegendDisplay() {
        ...
        if (legendItems.isEmpty()) {
            ...
            for (SeriesLegendItem item : legendItems) {
                seriesListPanel.add(item);
                seriesListPanel.add(Box.createVerticalStrut(2));
            }
        }
    ...
    public List<String> getVisibleSeries() {
        return legendItems.stream()
            .filter(SeriesLegendItem::isSeriesVisible)
            .map(SeriesLegendItem::getSeriesName)
            .toList();
    }
    ...
    private void setSelectedItem(SeriesLegendItem item) {
    ...
    public void moveSelectedItemUp() {
        if (selectedItem == null || legendItems.size() <= 1) return;
        
        int currentIndex = legendItems.indexOf(selectedItem);
        if (currentIndex <= 0) return; // Already at top or not found
        
        // Swap with previous item
        SeriesLegendItem item = legendItems.remove(currentIndex);
        legendItems.add(currentIndex - 1, item);
    ...
    public void moveSelectedItemDown() {
        ...
        int currentIndex = legendItems.indexOf(selectedItem);
        if (currentIndex < 0 || currentIndex >= legendItems.size() - 1) return; // Already at bottom or not found
    ...
    // Inner class for individual legend items
    private class SeriesLegendItem extends JPanel {
        ...
        public SeriesLegendItem(String seriesName, Color seriesColor, int pointCount, Runnable visibilityChangeCallback) {
