...
public class LoadSavePreferencePage extends AbstractPreferencePage {
    ...
    private JSpinner gzipMemoryLimitSpinner;
    ...
    private void initializePanel() {
        ...
        // Gzipped CSV in-memory limit: gzip is not seekable, so .csv.gz is
        ...
        gbc.weightx = 0; gbc.fill = GridBagConstraints.NONE;
        JLabel gzipLimitLabel = new JLabel("Open .csv.gz up to (decompressed MB):");
        gzipLimitLabel.setToolTipText("Gzipped CSV files (.csv.gz) open read-only, decompressed into "
            + "memory. Files whose decompressed size exceeds this limit are refused with the reason "
            + "shown in the tab.");
        formPanel.add(gzipLimitLabel, gbc);

        gbc.gridx = 1; gbc.gridy = 4;
        gzipMemoryLimitSpinner = new JSpinner(new SpinnerNumberModel(
            (int) PreferenceKeys.DATAVIEW_GZIP_MEMORY_LIMIT_MB.get(), 16, 2000, 128));
        gzipMemoryLimitSpinner.setToolTipText(gzipLimitLabel.getToolTipText());
        gzipMemoryLimitSpinner.addChangeListener(e ->
            PreferenceKeys.DATAVIEW_GZIP_MEMORY_LIMIT_MB.set((Integer) gzipMemoryLimitSpinner.getValue()));
        formPanel.add(gzipMemoryLimitSpinner, gbc);
