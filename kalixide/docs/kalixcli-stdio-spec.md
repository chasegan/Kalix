...
- Supported formats:
  - `"pixie"` (recommended): a compressed bitstream, base64-encoded into `r.data`. The bitstream carries timestep, count, and per-point timestamps, so no separate metadata is needed to decode. **All timestamps and the timestep in the bitstream are epoch SECONDS, and timestamps are plain signed 64-bit two's-complement Unix time (UTC)** — byte-for-byte the same convention as a `.pxb` file, and what POSIX `time_t`, `chrono::timestamp()` and Java's `Instant.getEpochSecond` mean. Dates before 1970 are negative. The engine's internal 2^63-biased ("offset-binary", `wrap_to_u64`) form never appears on the wire; it did before the engine and IDE were aligned, so a decoder that XORs the top bit is reading the old form. Consumers holding milliseconds must convert at this boundary. The codec is a custom delta/XOR scheme *inspired by* Facebook's Gorilla algorithm — it is not a literal Gorilla implementation. The wire `codec` identifier is `gorilla_double` for historical reasons.
...
{"m":"cmd","c":"get_result","p":{"series_name":"node.output1","format":"pixie"}}
{"m":"res","uid":"sess_20250908_103000_a7b9","cmd":"get_result","exec_ms":23.1,"ok":true,"r":{"series_name":"node.output1","format":"pixie","codec":"gorilla_double","data":"<base64 of compressed bytes>"}}
