...
- WATER ACCOUNTING & RESOURCE ALLOCATION SYSTEMS!
    - New `[acc.*]` section declares a group of user **accounts** as a simple table. See [`[acc.*]`](https://kalix.org/docs/accounts/).
    - New `[ras.*]` section defines **resource allocation system** events: *on this trigger, do this action, to these account groups*. Triggers can be based on the calendar (`start_water_year(m)`, `start_month`, …) or written as an expression; actions include `set_fraction`, `credit`, `debit`, and more. The action `allocate(pct)` assess resources and announces an allocation level as a percentage of entitlement for the group. See [`[ras.*]`](https://kalix.org/docs/ras/).
...
- MAJOR FUNCTIONS UPGRADE!
    ...
    - All the above is [documented here](https://kalix.org/docs/dynamic-expressions/).
- CALIBRATION STUFF:
