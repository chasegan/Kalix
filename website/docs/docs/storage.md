...
| forced\_level (optional) | Level [m] to pin the storage to at the end of the timestep. Its main use is storage inflow derivation (SID): supply observed levels, and the water the model must create or destroy to reach them is reported as `adjustment_volume`. On a timestep where it evaluates to a finite number the storage is not simulated: spill, area and outlet capacity are read at the forced level, releases are met subject to outlet capacity (not to the water available), and the difference is the adjustment. NaN means not forced on this timestep. Ordering is unchanged and evaluates against the forced level. Example: `forced_level = data.obs_csv.by_name.level` |
...
| adjustment\_volume | Water that forcing created (positive) or destroyed (negative) this timestep [ML]. |
| sid\_flux | Storage inflow derivation [ML]: `adjustment_volume` on the interior of a forced run; NaN on the first forced step and wherever not forced. With no modelled upstream inflow this is the derived inflow; with one, it is the residual — the derived total is `usflow + adjustment_volume`. |
